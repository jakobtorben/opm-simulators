...
struct NlddParallelColorStrategy { static constexpr auto value = "order"; };
...
public:
    ...
    /// Strategy for parallel coloring: "order" (use domain order) or "greedy" (greedy graph coloring)
    std::string nldd_parallel_color_strategy_{"order"};
