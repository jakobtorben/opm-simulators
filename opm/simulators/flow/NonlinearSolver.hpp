...
struct NonlinearSolverParameters
{
    ...
    public:
        ...
        SimulatorReportSingle step(const SimulatorTimerInterface& timer, const TimeStepControlInterface *timeStepControl)
        {
            ...
            while ( (!converged && (model_->iterationContext().globalIteration <= this->model().param().newton_max_iter_)) ||
                    (model_->iterationContext().globalIteration <= this->model().param().newton_min_iter_));
