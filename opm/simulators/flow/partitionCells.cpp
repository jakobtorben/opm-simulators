...
public:
    ...
    template <class GridView, class Element>
    void connectNeighbors(const GridView& grid_view,
                                        ...
                                        const int                                             num_neighbor_levels)
{
    ...
    this->connectWells(grid_view.comm(), grid_view, wells, possibleFutureConnections,
                      g2l, zoltan_ctrl, num_neighbor_levels);
}

template <class GridView, class Element>
void ZoltanPartitioner::connectNeighbors(const GridView& grid_view,
                                        std::vector<int>& cells,
                                        const int num_levels,
                                        const Opm::ZoltanPartitioningControl<Element>& zoltan_ctrl) const
{
    ...
    frontier.swap(cells);
    frontier.reserve(frontier.size() * 2);
    ...
    for (int level = 0; level < num_levels && !frontier.empty(); ++level) {
        ...
        frontier = std::move(new_frontier);  // Prepare next level
    }
                                     ...
                                     const int                                      num_neighbor_levels)
{
    ...
    for (const auto& well : wells) {
        ...
        if (num_neighbor_levels > 0 && !cellIx.empty()) {
            connectNeighbors(grid_view, cellIx, num_neighbor_levels, zoltan_ctrl);
        }
