         ...
         const Domain& domain)
{
    ...
    //
    // Call chain:  assemble() -> assembleWellEq(dt, domain)
    //                -> well->assembleWellEq()          [WellInterface]
    //                   -> prepareWellBeforeAssembling() [WellInterface]
    //                   -> assembleWellEqWithoutIteration()
    //
    // Key assumptions and design decisions:
    //
    // 1. Global well initialization (prepareTimeStep, including the optional
    //    initial well solve) has already been called via wellModel_.assemble()
    //    during the initial global linearization.
    //    This is ensured by NewtonIterationContext::needsTimestepInit().
    //
    // 2. Inner well iterations (iterateWellEquations) are skipped during local
    //    solves via shouldRunInnerWellIterations() (inLocalSolve=true).
    //    The NLDD local Newton loop already iterates wells to convergence.
    //
    // 3. No MPI collective operations (do_mpi_gather=false).
    //
    // 4. Only individual well constraints are checked (see updateWellControls).
    ...
    //      checks (checkIndividualConstraints) never produce GRUP as a result;
    ...
    // pipeline: updateAndCommunicateGroupData, updateGroupControls,
